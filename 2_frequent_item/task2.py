...
if __name__ == '__main__':
    ...
    basket = input_file.map(lambda x : x.split(',')).map(lambda x:(x[0], x[1])).distinct().map(lambda x: (x[0], [x[1]])).reduceByKey(add).persist().filter(lambda x: x[0]!= "user_id").map(lambda x: set(x[1])).filter(lambda x: len(x) > int(filter_threshold))
    total_item= list(set().union(*basket.collect()))
    sup_part = int(support)/input_file.getNumPartitions()
    
    son_pass1 = basket.mapPartitions(lambda x: apriori(x, total_item, sup_part)).map(lambda x: (tuple(x),1)).distinct().sortByKey().map(lambda x: x[0]).sortBy(lambda x: (len(x), x))
